...
import static org.services.DatabaseInfo.DB_URL;
...
public class PostService {
    ...
    public static void savePost(Post post) {
        ...
        Statement statment = connection.createStatement() ){
            ...
            statment.executeUpdate(SQL_savePost);

        }catch (SQLException e){
            System.out.println("Error, your post don't save!");
        }
    }
    ...
    public static List<Post> getPostsByUser(User user) {
        String SQL_postsByUser = "SELECT * FROM "+ DatabaseInfo.Tables.POSTS + " WHERE USER_ID = "
                + "\"" + user.getIdString() + "\" ORDER BY POST_DATE DESC;" ;
        List<Post> userStories = new ArrayList<>();

        try(Connection connection = DriverManager.getConnection(DB_URL);
        Statement statment = connection.createStatement();
        ResultSet resultSet = statment.executeQuery(SQL_postsByUser) ){
            ...
            while (resultSet.next()) {
                Post post = new Post(resultSet.getString(1),
                        ...
                        user,
                        resultSet.getLong(5), null);

                userStories.add(post);
            }
        }catch (SQLException e){
            System.out.println("Error, falling to retreive user's posts!");
        }
    ...
    public static List<Post> mostRecentNews() {
        ...
        try(Connection connection = DriverManager.getConnection(DB_URL);
        Statement statment = connection.createStatement();
        ResultSet resultSet = statment.executeQuery(SQL_recentNews);){

            while (resultSet.next()) {
                User user = UserService.getUserByID(resultSet.getString(4));
                List<Comment> comments = PostService.getCommentsForPost(resultSet.getString(1));
                Post post = new Post(resultSet.getString(1), resultSet.getString(2),
                        resultSet.getString(3), user, resultSet.getLong(5), comments);
            ...
            System.out.println("Error,an exception happened!");
        }
